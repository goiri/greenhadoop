...
for line in open(LOG_ENERGY, "r"):
	if line.startswith("#"):
		...
		# Options = {'workloadFile': 'workload/workload-continuous-030', 'greenFile': 'data/solarpower-09-05-2011', 'speedup': 24.0, 'taskEnergy': 0.34999999999999998, 'schedNoBrown': None, 'predGreen': True, 'deadline': 900, 'schedNoGreen': None, 'peakCost': 13.6136, 'brownFile': 'data/browncost-onoffpeak-winter.nj', 'date': '2011-5-9T00:00:00', 'replThreads': 30, 'schedEnergy': True}
	elif line!="\n":
		line = line.replace("\n", "")
		...
		if prevLineSplit != None:
			...
			# TODO fix
			if False:
				if energyTotal < t*(4*70.0 + 12*8.6):
					energyTotal = t*(4*70.0 + 12*8.6)
			energyBrown = energyTotal - energyGreen
...
for line in open(LOG_JOBS, "r"):
	if not line.startswith("#") and line!="\n":
		...
		if jobId not in jobsStartEnd:
			jobsStartEnd[jobId] = (lineSplit[5], None, lineSplit[4], 0)
			
		jobsStartEnd[jobId] = (jobsStartEnd[jobId][0], jobsStartEnd[jobId][1], jobsStartEnd[jobId][2], jobsStartEnd[jobId][3]+1)
		...
		if lineSplit[3].startswith("['"):
			...
			jobsStartEnd[jobId] = (jobsStartEnd[jobId][0], lineSplit[10], lineSplit[4], jobsStartEnd[jobId][3])
